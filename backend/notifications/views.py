...
from notifications.models import FeedbackItem, WebhookSubscription
from notifications.serializers import (
	...
	PublicWebPushUnsubscribeSerializer,
	VkOAuthStartResponseSerializer,
	WebPushPublicKeySerializer,
	WebPushUnsubscribeResponseSerializer,
...
class PublicWebPushUnsubscribeView(APIView):
	authentication_classes = []
	permission_classes = []

	@extend_schema(
		request=PublicWebPushUnsubscribeSerializer,
		responses={status.HTTP_200_OK: WebPushUnsubscribeResponseSerializer},
	)
	def post(self, request):
		serializer = PublicWebPushUnsubscribeSerializer(data=request.data)
		serializer.is_valid(raise_exception=True)
		disabled_count = unsubscribe_web_push(
			queue_id=serializer.validated_data['queue_id'],
			client_id=serializer.validated_data.get('client_id'),
			ticket_id=serializer.validated_data.get('ticket_id'),
			endpoint=serializer.validated_data.get('endpoint') or None,
		)
		output_serializer = WebPushUnsubscribeResponseSerializer(
...
class AdminFeedbackItemViewSet(
