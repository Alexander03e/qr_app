...
export const OperatorLogin = () => {
  ...
  const { mutateAsync, isPending } = useMutation({
    mutationFn: async (payload: { email: string; password: string }) =>
      operatorAuth.login(payload.email, payload.password),
  });

  const onFinish = async (values: { email: string; password: string }) => {
    try {
      const response = await makeRequest(mutateAsync(values));
      operatorAuth.writeToken(response.token);
      const redirectTo = searchParams.get("next") ?? "/o/1";
      navigate(redirectTo, { replace: true });
    } catch {
      message.error(t("operator.login.invalid"));
    }
  };

  return (
    <Card style={{ width: 420, margin: "56px auto" }}>
      <Typography.Title level={3}>{t("operator.login.title")}</Typography.Title>
      <Form layout="vertical" onFinish={onFinish}>
        <Form.Item
          name="email"
          label={t("operator.login.email")}
