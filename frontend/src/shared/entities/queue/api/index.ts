...
class QueuesApi {
  ...
  async getQueueSnapshot(
    ...
    return (
      await $api.get(`/queues/${queueId}/snapshot/`, {
  ...
  async deleteTicketsFromQueue(
    ...
    return (await $api.post(`/queues/${queueId}/tickets/delete/`, payload))
      .data;
  }

  async getTickets(
    params?: QueueTicketListRequest
  ...
  async skipOneAhead(ticketId: number): Promise<AppendToQueueResponse> {
    return (await $api.post(`/tickets/${ticketId}/skip-one-ahead/`)).data;
  }

  async inviteTicketById(
    ticketId: number,
    payload?: InviteByIdRequest
  ): Promise<TicketStatusUpdateResponse> {
    return (await $api.post(`/tickets/${ticketId}/invite/`, payload ?? {}))
      .data;
  }

  async removeTicket(ticketId: number): Promise<TicketStatusUpdateResponse> {
    return (await $api.post(`/tickets/${ticketId}/remove/`)).data;
  }
}
